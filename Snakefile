...
rule target:
	input:
		...
		'output/sample_trinity_abundance/all_salmon.isoform.TPM.not_cross_norm'

############################
##sample trinity abundance##
############################

##to compare expression between samples without replicates
rule sample_trinity_abundance_to_matrix:
	input:
		gt_map = 'output/trinity/Trinity.fasta.gene_trans_map',
		abundance_files = expand('output/sample_trinity_abundance/{sample}/quant.sf', sample=all_samples)
	output:
		'output/sample_trinity_abundance/all_salmon.isoform.counts.matrix',
		'output/sample_trinity_abundance/all_salmon.isoform.TPM.not_cross_norm'
	params:
		prefix = 'output/sample_trinity_abundance/abundance_matrix/all_salmon'
	singularity:
		trinity_container
	log:
		'output/logs/sample_abundance/abundance_estimates_to_matrix.log'
	shell:
		...
		'{input.abundance_files} '
		'&> {log}'

rule sample_trinity_abundance:
	input:
		transcripts = 'output/trinity/Trinity.fasta',
		left = 'output/bbduk_trim/{sample}_r1.fq.gz',
		right = 'output/bbduk_trim/{sample}_r2.fq.gz'
	output:
		'output/sample_trinity_abundance/{sample}/quant.sf'
	singularity:
		trinity_container
	threads:
		20
	log:
		'output/logs/sample_abundance/{sample}_trinity_abundance.log'
	params:
		outdir = 'output/sample_trinity_abundance/{sample}'
	shell:
